...
def test_lcs_reorder_no_cascade_on_insertion_removal() -> None:
    before = parse_standalone_vsys1(
        load_xml_file(
            str(Path(__file__).resolve().parents[1] / "fixtures" / "before_basic.xml")
        ).root
    )
    after = parse_standalone_vsys1(
        load_xml_file(
            str(Path(__file__).resolve().parents[1] / "fixtures" / "after_basic.xml")
        ).root
    )
